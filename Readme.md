...
一个基于AI的数学概念可视化工具，能够将抽象的数学概念转化为直观的教学动画。

## ✨ 特性

- **AI驱动的教学设计**：自动分析数学概念，生成针对性的教学方案
- **实时动画生成**：将数学概念转换为Manim动画代码并实时渲染
- **三维可视化**：支持2D/3D空间展示，自动选择最适合的表现形式
- **智能分镜系统**：自动将复杂概念分解为易于理解的场景序列
- **渐进式教学**：从简单到复杂，循序渐进地展示数学概念
- **动态视角控制**：支持多角度观察，增强空间理解
- **可配置渲染质量**：支持不同分辨率和帧率的渲染设置
...
1. 首次运行需要安装LaTeX环境
2. 建议使用独立显卡以获得更好的渲染性能
3. 高质量渲染可能需要较长时间
4. 确保有稳定的网络连接以访问AI API
